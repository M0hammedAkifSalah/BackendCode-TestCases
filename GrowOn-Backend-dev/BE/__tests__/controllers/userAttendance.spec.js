...
describe('UserAttendance Controller', () => {
		...
		it('should return school not registered error', async () => {
			...
			const req = (mockRequest().body = {
				...
				body: {
					longitude: 0.12,
					latitude: 0.23,
					school_id: '62333c7a333f39c372823239',
				},
			});
			const res = mockResponse();
			await verifyLocation(req, res, mockNext);
	...
	describe('report conttroller method', () => {
		it('should return input validation error', async () => {
			const req = (mockRequest().body = { query: {} });
			const res = mockResponse();
			await report(req, res, mockNext);
			expect(mockNext).toHaveBeenCalledTimes(1);
			expect(res.status).toHaveBeenCalledWith(422);
			expect(res.json).toHaveBeenCalled();
			expect(ErrorResponse).toHaveBeenCalledWith('Invalid input', 422);
		});
		it('should return invalid date error', async () => {
			const req = (mockRequest().body = {
				query: {
					schoolId: '5f55f5f5f5f5f5f5f5f5f5',
					date: 'dummy',
				},
			});
			const res = mockResponse();
			...
			expect(res.status).toHaveBeenCalledWith(422);
			expect(res.json).toHaveBeenCalled();
			expect(ErrorResponse).toHaveBeenCalledWith('Invalid date', 422);
		});
		it('should return no data found', async () => {
			jest.spyOn(userAttendanceReport, 'find').mockImplementationOnce(() => ({
			...
			expect(res.json).toHaveBeenCalled();
			expect(ErrorResponse).toHaveBeenCalledWith('No data found', 404);
		});
		it('should return no data found', async () => {
			jest.spyOn(userAttendanceReport, 'find').mockImplementationOnce(() => ({
				populate: jest.fn().mockResolvedValueOnce([
